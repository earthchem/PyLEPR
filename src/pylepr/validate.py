...
def _validate_chem_method(chem_dat_info, format: ChemDataInfo):
    for col_ind, (col, dat) in enumerate(chem_dat_info.T.iterrows()):
        ...
        if dat.method_id is np.nan:
            logging.critical(
                f"<<cell {cell_id}>>: '{col}' does not provide any method id"
            )


def validate_chem_data_info(chem_dat_info, format=CHEM_DATA_INFO):
...
def _chem_not_detected_not_valid(cell_data, format: ChemDataInfo):
    val, chem, run_id = [cell_data[key] for key in ["val", "chem", "run_id"]]

    cell_id = _get_chem_data_cell(cell_data, format)

    if val == "nd":
        logging.error(
            f"<<cell {cell_id}>>: '{val}', the '{chem}' value for exp_run '{run_id}' is not valid. If not detected use vocabulary 'bdl'"
        )
        return True
...
def _chem_not_measured_not_valid(cell_data, format: ChemDataInfo):
    val, chem, run_id = [cell_data[key] for key in ["val", "chem", "run_id"]]
    ...
    if val == "-":
        logging.error(
            f"<<cell {cell_id}>>: '{val}', the '{chem}' value for exp_run '{run_id}' is not valid. If not measured leave entry blank"
        )
        return True
...
def _chem_measurement_limit_not_valid(cell_data, format: ChemDataInfo):
    val, chem, run_id = [cell_data[key] for key in ["val", "chem", "run_id"]]
    ...
    if val.startswith(">") or val.startswith("<"):
        logging.error(
            f"<<cell {cell_id}>>: '{val}', the '{chem}' value for exp_run '{run_id}' is not valid. Instead give just the value and indicate limit using field '????, Ask roger'"
        )
        return True
...
def _get_chem_data_cell(cell_data, format: ChemDataInfo):
    col_num = format.chem_dat_col_index + cell_data["col_ind"] + 1
    col_str = get_column_letter(col_num)
    row_num = (
        format.header_row_num
        + format.metadata_header_row_num
        + cell_data["row_ind"]
        + 1
    )
...
def _numeric_chem_data_not_valid(cell_data, format: ChemDataInfo):
    val, chem, run_id = [cell_data[key] for key in ["val", "chem", "run_id"]]

    cell_id = _get_chem_data_cell(cell_data, format)
    if type(val) is str:
        logging.error(
            f"<<cell {cell_id}>>: '{chem}' value for exp_run '{run_id}' is invalid. '{val}' is not a valid number."
        )
        return True
